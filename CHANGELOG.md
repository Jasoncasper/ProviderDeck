...
- 修复点击启动 ChatGPT 后立即自动关闭：当 ChatGPT 已运行但未开启 CDP 调试端口时，macOS `open -a` 仅激活旧实例并忽略 `--remote-debugging-port`，导致注入连不上 CDP、触发回滚关闭与 watcher 反复接管。改为先退出旧实例并等待其退出，再以带调试端口的方式重启，确保 CDP 可达。
