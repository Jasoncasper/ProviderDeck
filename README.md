...
ProviderDeck 与 CodexMate 是两个独立软件，使用不同的应用名、bundle id、端口、状态目录和更新源。ProviderDeck 失败时可退出后单独启动 CodexMate v1.0.9。
...
bash scripts/installer/macos/package-dmg.sh 0.1.0 aarch64
```
...
旧 CodexMate 配置导入只读取 `settings.json` 和 `routing.toml`，不会读取或复制历史、备份、Plugins、Skills 或 MCP 配置。详见 [Provider 配置](docs/provider-configuration.md)、[切换行为](docs/model-switching.md) 和 [故障恢复](docs/recovery.md)。
