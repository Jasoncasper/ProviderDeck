...
pub fn scope_request_for_provider(provider_id: &str, body: &str) -> anyhow::Result<String> {
    crate::provider_catalog::validate_provider_id(provider_id)?;
    let mut request: Value = serde_json::from_str(body)?;
    let model = request
        .get("model")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow::anyhow!("responses request is missing model"))?;
    request["model"] = Value::String(format!("{provider_id}:{model}"));
...
fn provider_user_agent(provider: &crate::router::config::SmartProvider) -> String {
    ...
    if configured.is_empty() {
        format!("ProviderDeck/{}", env!("CARGO_PKG_VERSION"))
    } else {
...
fn load_default_router_provider() -> anyhow::Result<crate::router::config::SmartProvider> {
    ...
    if config_path.exists() {
        if let Ok(contents) = std::fs::read_to_string(&config_path) {
            if let Ok(config) =
                toml::from_str::<crate::router::config::SmartRouterConfig>(&contents)
            {
...
pub async fn open_responses_proxy_request(body: &str) -> anyhow::Result<UpstreamProxyResponse> {
    ...
    // 图片自动压缩
    let compress_config = crate::image_compressor::ImageCompressConfig::default();
    let _compress_result =
        crate::image_compressor::compress_body_images(&mut request_json, &compress_config);
    ...
    if config_path.exists() {
        if let Ok(contents) = std::fs::read_to_string(&config_path) {
            if let Ok(config) =
                toml::from_str::<crate::router::config::SmartRouterConfig>(&contents)
            {
                let config = crate::router::config::normalize_router_config(config);
                model_ids = config
                    .providers
                    .iter()
                    .filter(|p| p.enabled && !p.id.trim().is_empty())
                    .map(crate::router::config::scoped_model_id)
                    .collect();
            }
        }
    }
...
/// 使用智能路由发起代理请求
pub async fn open_provider_proxy_request(
    provider_id: &str,
    body: &str,
) -> anyhow::Result<(UpstreamProxyResponse, String)> {
    let scoped = scope_request_for_provider(provider_id, body)?;
    open_routed_proxy_request(&scoped).await
}

pub async fn open_routed_proxy_request(
    body: &str,
) -> anyhow::Result<(UpstreamProxyResponse, String)> {
    let mut request_json: Value = serde_json::from_str(body)?;
    ...
    let (decision, router) = if router_config_path.exists() {
    ...
    if let Some(ref router) = router {
        ...
        if cfg.image_auto_compress {
            let compress_config = crate::image_compressor::ImageCompressConfig {
                enabled: true,
                max_size_kb: cfg.image_max_size_kb,
...
/// 从 chat request 的 messages 中剥离 image_url 内容（上游报错后重试用）
fn strip_image_content_from_chat_request(chat_request: &mut Value) {
    ...
    for message in messages {
        strip_image_from_message_content(message);
    }
}

/// 从单条 message 的 content 中移除 image_url 类型部分
fn strip_image_from_message_content(message: &mut Value) {
    ...
    if text_parts.is_empty() {
        *content = Value::String(String::new());
        ...
        && text_parts[0].get("type").and_then(Value::as_str) == Some("text")
    {
        if let Some(text) = text_parts[0].get("text").and_then(Value::as_str) {
            *content = Value::String(text.to_string());
        } else {
            *content = Value::Array(text_parts);
        }
    } else {
        *content = Value::Array(text_parts);
    }
...
mod provider_request_tests {
    ...
    fn provider_request_uses_custom_user_agent_and_bearer_token() {
        let provider = SmartProvider {
            ...
            builtin: false,
            user_agent: "ProviderDeck-Test/1.0".to_string(),
            max_context: 0,
    ...
    #[test]
    fn provider_request_uses_providerdeck_user_agent_by_default() {
        let provider = SmartProvider {
            id: "third-party".to_string(),
            name: "Third Party".to_string(),
            base_url: "https://api.example.com/v1".to_string(),
        ...
        assert_eq!(
            request.headers().get(reqwest::header::USER_AGENT).unwrap(),
            concat!("ProviderDeck/", env!("CARGO_PKG_VERSION"))
        );
    }
}
