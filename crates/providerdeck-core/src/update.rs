...
pub fn configured_repository() -> Option<&'static str> {
    option_env!("CARGO_PKG_REPOSITORY").and_then(normalize_github_repository)
}

pub fn configured_latest_json_url() -> Option<String> {
    configured_repository()
        .map(|repo| format!("https://github.com/{repo}/releases/latest/download/latest.json"))
}

fn normalize_github_repository(value: &str) -> Option<&str> {
    let trimmed = value.trim().trim_end_matches('/');
    if trimmed.is_empty() {
        return None;
    }
    if trimmed.contains("your-github-username") || trimmed.contains("<your-github-username>") {
        return None;
    }
    trimmed
...
pub async fn fetch_latest_release(latest_json_url: &str) -> anyhow::Result<Release> {
    let client =
        crate::http_client::proxied_client(&format!("ProviderDeck/{}", crate::version::VERSION))?;
    let payload = client
        .get(latest_json_url)
...
pub async fn check_for_update(current_version: &str) -> anyhow::Result<UpdateCheck> {
    let latest_json_url = configured_latest_json_url().ok_or_else(|| {
        anyhow::anyhow!(
            "未配置 GitHub 仓库地址，请先在 Cargo.toml 的 workspace.package.repository 中设置真实仓库 URL"
        )
    })?;
    let release = fetch_latest_release(&latest_json_url).await?;
...
fn is_macos_installer_asset(name: &str) -> bool {
    name.contains("providerdeck") && name.ends_with(".dmg")
}
