...
fn bridge_script_defines_expected_globals_and_binding() {
    ...
    assert!(script.contains("window.__providerDeckBridge"));
    assert!(script.contains("window.__providerDeckResolve"));
    assert!(script.contains("window.__providerDeckReject"));
    assert!(script.contains("providerDeckBridgeV1"));
}
...
fn injection_script_prefixes_runtime_catalog_and_version_globals() {
    ...
    assert!(script.contains("window.__PROVIDERDECK_BOOTSTRAP__"));
    assert!(script.contains("\"providers\""));
    assert!(script.contains("window.__PROVIDERDECK_VERSION__"));
    assert!(script.contains(providerdeck_core::version::VERSION));
}

#[test]
fn injection_script_uses_narrow_app_server_coordination() {
    let script = assets::injection_script(57421);
...
fn bridge_health_check_script_uses_real_backend_round_trip() {
    ...
    assert!(script.contains("__providerDeckBridge"));
...
fn bridge_result_expressions_json_escape_inputs() {
    ...
    assert_eq!(
        resolve,
        r#"window.__providerDeckResolve("request\"1", {"status":"ok"})"#
    );
    assert_eq!(
        reject,
        r#"window.__providerDeckReject("request\"1", "bad \"value\"")"#
    );
}
...
#[test]
fn pick_page_target_prefers_chatgpt_title_or_url() {
    let targets = vec![
        target(
            "first",
            "page",
...
fn pick_page_target_rejects_non_pages_and_pages_without_websocket() {
    ...
    assert!(
        error
            .to_string()
            .contains("No injectable Codex/ChatGPT page target found")
    );
}
    ...
    let (url, request_rx) = spawn_cdp_server(|mut socket| async move {
        ...
        assert!(
            response["params"]["expression"]
                ...
                .contains("__providerDeckResolve")
        );
...
fn assert_expression_contains_request(command: &serde_json::Value, request_id: &str) {
    let expression = command["params"]["expression"]
        .as_str()
        .expect("expression should be string");
    assert!(expression.contains("__providerDeckResolve"), "{expression}");
