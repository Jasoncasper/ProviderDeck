...
use providerdeck_core::app_paths::{
...
use providerdeck_core::ports::select_platform_loopback_port_with;
use providerdeck_core::proxy::has_proxy_environment;
use providerdeck_core::settings::BackendSettings;
use providerdeck_core::status::StatusStore;
...
fn app_paths_find_macos_codex_app_supports_chatgpt_bundle() {
    ...
    let classic_app = root.join("ChatGPT Classic.app");
    std::fs::create_dir_all(&classic_app).unwrap();
    std::fs::create_dir_all(&chatgpt_app).unwrap();

    assert_eq!(find_macos_codex_app(&[root]).unwrap(), chatgpt_app);
}
...
#[test]
fn app_paths_build_macos_bundle_executable_uses_bundle_plist() {
    let temp = tempfile::tempdir().unwrap();
...
#[test]
fn ports_non_windows_also_falls_back_when_requested_is_busy() {
...
async fn default_helper_accepts_diagnostic_log_events_over_http() {
    let temp = tempfile::tempdir().unwrap();
    let log_path = temp.path().join("providerdeck.log");
    providerdeck_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    let hooks = DefaultLaunchHooks::default();
    let listener = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    drop(listener);
    ...
    providerdeck_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}

#[tokio::test]
async fn helper_rejects_unauthenticated_provider_requests_and_has_no_global_responses_route() {
    let hooks = DefaultLaunchHooks::default();
    let listener = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    drop(listener);
    ...
    let scoped = client
        .post(format!(
            "http://127.0.0.1:{port}/provider/team/v1/responses"
        ))
        .json(&serde_json::json!({"model":"vendor-model"}))
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn launch_lifecycle_writes_success_and_shutdowns_on_exit() {
    let temp = tempfile::tempdir().unwrap();
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_launch_result(CodexLaunch::Process {
            ...
            wait_strategy: providerdeck_core::launcher::ProcessWaitStrategy::TrackedChild,
