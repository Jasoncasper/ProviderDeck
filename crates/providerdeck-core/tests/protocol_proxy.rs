use providerdeck_core::protocol_proxy::{
...
#[test]
fn provider_proxy_path_extracts_only_valid_provider_ids() {
    assert_eq!(
        parse_provider_proxy_path("/provider/team_proxy/v1/responses"),
        Some("team_proxy")
    );
    assert_eq!(
        parse_provider_proxy_path("/provider/bad:id/v1/responses"),
        None
    );
...
#[test]
fn provider_request_is_scoped_before_router_selection() {
    let scoped = scope_request_for_provider(
        "team_proxy",
        r#"{"model":"vendor:model:v2","input":"hello"}"#,
    )
    .unwrap();
    let payload: serde_json::Value = serde_json::from_str(&scoped).unwrap();

    assert_eq!(payload["model"], "team_proxy:vendor:model:v2");
